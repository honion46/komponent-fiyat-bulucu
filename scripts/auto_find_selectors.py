...
def selector_from_element(el):
    if el.name is None: return None
    if el.get('id'): return f"#{el.get('id')}"
    classes = el.get('class') or []
    if classes: return f"{el.name}.{classes[0]}"
    p = el.parent
    while p and p.name is not None and p.name != '[document]':
        if p.get('id'): return f"#{p.get('id')} {el.name}"
        pcls = p.get('class') or []
        if pcls: return f"{p.name}.{pcls[0]} {el.name}"
        p = p.parent
    return el.name

def test_selector_uniqueness(soup, selector, expected_text):
    try: sel = soup.select(selector)
    except Exception: return False
    if not sel: return False
    for el in sel:
        if expected_text in el.get_text(separator=" ", strip=True): return True
    return False

def detect_for_site(name, conf):
    url_template = conf.get("url_template"); suggestions=[]
    for sku in SAMPLE_SKUS:
        query = quote_plus(sku); url = url_template.format(sku=query)
        try:
...
def main():
    ...
    for k,v in out.items():
        ...
        if isinstance(v,list) and v:
            for cand in v:
                if cand.get('unique'): sel=cand.get('selector'); break
            if not sel: sel=v[0].get('selector')
